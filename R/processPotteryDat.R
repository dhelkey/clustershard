processPotteryDat = function(filelement_names_path, id_column = 2, 
	...
	stopifnot(element_start_column > 1)
	...
	id_run_list = stringr::str_split(dat_raw[ ,id_column],'-')
    ...
    dat = dat_raw[ , c('id', 'run', element_names)]
    names(dat) = c('id', 'run', element_names_p)
